const paragraphBox = document.querySelector(".paragraph");
...
function loadParagraph() {
  ...
  const characters = paragraph[randomPara].split("");

  paragraphBox.innerHTML = "";

  characters.forEach((char) => {
    paragraphBox.innerHTML += `<span>${char}</span>`;
  });
...
inputField.addEventListener("input", function () {
  ...
  const allSpans = paragraphBox.querySelectorAll("span");

  if (typedText.length > charIndex) {
    const typedChar = typedText.charAt(charIndex);
    const currentSpan = allSpans[charIndex];
    ...
    charIndex--; 
    const prevSpan = allSpans[charIndex];
...
function calculateWPM(params) {
   let wordsTyped = (charIndex - mistakes) / 5
   let minutes = (timeLimit - timeRemaining) / 60
    if (minutes > 0) {
